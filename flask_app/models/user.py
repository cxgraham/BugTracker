...
class User:
    ...
    def get_user_by_id(cls, user_id):
        ...
        query = """
        SELECT * FROM users
        LEFT JOIN bugs ON users.id = bugs.user_id
        WHERE users.id = %(id)s
        ;"""
        ...
        if results:
            ...
            for this_user in results:
                bug_data = {
                    'id': this_user['bugs.id'],
                    'title': this_user['title'],
                    'description': this_user['description'],
                    'status': this_user['status'],
                    'priority': this_user['priority'],
                    'date': this_user['date'],
                    'user_id': this_user['user_id'],
                    'created_at': this_user['bugs.created_at'],
                    'updated_at': this_user['bugs.updated_at']
                }
                user.bugs.append(bug.Bug(bug_data))
        return user
